...
(function(angular) {
	...
	function PropertyUtil() {
	 ...
	 /* @ngInject */
	angular.module("nDaterangepicker").service(
			...
			function() {
				...
				this.isValidDateType = function(type) {
					if (-1 == this.availableDateTypes.indexOf(type)) {
						throw new TypeError("Got type (" + type
								+ ") and expected one of the following ("
								+ this.availableDateTypes.join(",") + ")");
					}
					return true;
				};
	...
	angular
			.module("nDaterangepicker")
			 /* @ngInject */
			.directive(
					"dateRangePicker",
					function($timeout, DateRangePickerService,
							DateRangePickerLocaleService, $log) {
						return {
							restrict : "A",
							require : "^ngModel",
							scope : {
							...
							controller : function($scope, $parse) {
								var locale = angular.extend({},
										DateRangePickerLocaleService.getList());
								if ($scope.options) {
									$scope.options = $parse($scope.options)();
								}
								if ($scope.options && $scope.options.locale) {
									locale = angular.extend(locale,
											$scope.options.locale);
									delete $scope.options.locale;
								}
								$scope.internalOptions = angular.extend({},
										DateRangePickerService.getList(), {
											locale : locale
										}, $scope.options);
								DateRangePickerService
										.isValidDateType($scope.internalOptions.type);
							},
							 /* @ngInject */
							link : function(scope, iElement, iAttrs,
								...
								if (scope.options && scope.options.identifier) {
									scope.$on(scope.options.identifier
											+ "Reset", function() {
										...
										.push(function(viewValue) {
											...
											if (scope.internalOptions.singleDatePicker) {
												...
												return _toType(viewValue);
													...
													.isObject(viewValue)
													|| !(viewValue
															.hasOwnProperty("startDate") && viewValue
															.hasOwnProperty("endDate"))) {
												return _toType(ngModelCtrl.$modelValue);
											}
											return _toType(viewValue);
										});
								ngModelCtrl.$validators.dateRequired = function(
										modelValue) {
									if (angular
											.isUndefined(iAttrs.dateRequired)) {
								...
								ngModelCtrl.$validators.notLaterThan = function(
										modelValue) {
									if (angular
											.isUndefined(iAttrs.notLaterThan)) {
										return true;
									}
									...
									var prepareDateString = iAttrs.notLaterThan
											.replace(/^"|"$/g, ""), momentDate = moment(
											prepareDateString,
											scope.internalOptions.format);
									if (!momentDate.isValid()) {
										momentDate = moment(prepareDateString,
												scope.internalOptions.isoFormat);
									}
									if (!momentDate.isValid()) {
										throw new Error(
												'Passed in comparison model for "notLaterThan" validator is invalid!');
									}
									var modelAsMoment = _getMoment(modelValue);
									return modelAsMoment.isBefore(momentDate)
											|| modelAsMoment.isSame(momentDate);
								};
								ngModelCtrl.$validators.notEarlierThan = function(
										modelValue) {
									if (angular
											.isUndefined(iAttrs.notEarlierThan)) {
										return true;
									}
									if (angular.isUndefined(modelValue)
											|| modelValue === null) {
									...
									var prepareDateString = iAttrs.notEarlierThan
											.replace(/^"|"$/g, ""), momentDate = moment(
											prepareDateString,
											scope.internalOptions.format);
									if (!momentDate.isValid()) {
									...
									if (!momentDate.isValid()) {
										throw new Error(
												'Passed in comparison model for "notEarlierThan" validator is invalid!');
									}
									var modelAsMoment = _getMoment(modelValue);
									return modelAsMoment.isAfter(momentDate)
											|| modelAsMoment.isSame(momentDate);
								};
								...
								ngModelCtrl.$render = function() {
									if (!_isAcceptableDate(_userInput)) {
										return el
												.val(_lastCatchableEvent !== "blur" ? _userInput
														: function() {
															var result;
															if (moment
																	.isMoment(_tmpModelPlaceholder)) {
																result = _tmpModelPlaceholder
																		.format(scope.internalOptions.format);
															} else {
									...
									if (!ngModelCtrl.$modelValue
											|| ngModelCtrl.$modelValue.startDate === null) {
										return el.val("");
									}
									_validate(ngModelCtrl.$modelValue);
									return el
											.val(_formatted(ngModelCtrl.$modelValue));
								};
								_init = function(initialValue) {
									if (!scope.internalOptions.singleDatePicker) {
										throw new Error(
												"Currently only single datepicker is supported!");
									}
											...
											.daterangepicker(
													...
													function(start, end) {
														return $timeout(function() {
															return scope
																	.$apply(function() {
																		var userInputAsMoment = angular
																				.isDefined(_persistentUserInput)
																				&& _persistentUserInput.length > 0 ? moment(
																		...
																		if (moment
																				...
																				&& end
																						.isValid()) {
																			if (angular
																					.isUndefined(userInputAsMoment)
																					|| userInputAsMoment
																					&& start
																							.diff(userInputAsMoment)) {
																				_setViewValue(
																						start,
																						end);
																			}
								...
								_setViewValue = function(startDate, endDate) {
									if (!scope.internalOptions.singleDatePicker) {
										ngModelCtrl
												.$setViewValue({
													startDate : startDate !== null ? _toType(startDate)
															: null,
													endDate : endDate !== null ? _toType(endDate)
															: null
												});
									} else {
										ngModelCtrl
												.$setViewValue(startDate !== null ? _toType(startDate)
														: null);
									}
								};
								_setIsSingleDatePicker = function(value) {
									if (value) {
										scope.internalOptions.singleDatePicker = !value
								...
								_setRange = function(startDate, endDate) {
									...
									if (!_getMoment(startDate).isValid()) {
										throw new Error(
												"Either invalid startDate was passed or invalid format. Please check!");
									}
									if (!_getMoment(endDate).isValid()) {
										throw new Error(
												"Either invalid endDate was passed or invalid format. Please check!");
									}
								...
								_formatted = function(viewVal) {
									...
									if (scope.internalOptions.singleDatePicker) {
										...
										result = [ f(viewVal.startDate),
												f(viewVal.endDate) ]
												.join(scope.internalOptions.separator);
									}
									return result;
								};
								...
								_validate = function(value) {
									var startDate = angular
											.isDefined(value.startDate) ? value.startDate
											: value, endDate = angular
											.isDefined(value.endDate) ? value.endDate
											: value;
									if (scope.internalOptions.minDate
											&& startDate) {
										_validateMin(
												scope.internalOptions.minDate,
												startDate);
									} else {
										ngModelCtrl.$setValidity("minDate",
												true);
									}
									if (scope.internalOptions.maxDate
											&& endDate) {
										_validateMax(
												scope.internalOptions.maxDate,
												endDate);
									} else {
										ngModelCtrl.$setValidity("maxDate",
												true);
									}
								};
								_validateMin = function(min, start) {
									...
									valid = min.isBefore(start)
											|| min.isSame(start, "day");
								...
								_isAcceptableDate = function(date) {
									...
									return new RegExp(
											scope.internalOptions.formatMask,
											"g").test(date)
											&& _getMoment(date).isValid();
								};
								el.on("apply.daterangepicker", function(e,
										picker) {
									_lastCatchableEvent = e.type;
									return $timeout(function() {
										return scope.$apply(function() {
											_setViewValue(picker.startDate,
													picker.endDate);
											return ngModelCtrl.$render();
										});
									});
										...
										.on(
												...
												function(e) {
													...
													if (ngModelCtrl.$modelValue === null
															|| _isAcceptableDate(ngModelCtrl.$modelValue)) {
								...
								el
										.on(
												...
												function(e) {
													_lastCatchableEvent = e.type;
													if (angular
															.element(".daterangepicker.show-calendar:visible").length > 0
															&& _tmpModelPlaceholder === null) {
														return;
													}
								...
								angular
										...
										.on(
												...
												function(e) {
													var innerEl = angular
															.element(e.target), closestTable = innerEl
															.closest("table"), closestTableParent;
													if (angular
															.isDefined(closestTable)
															&& closestTable.length) {
														closestTableParent = closestTable
																.parent();
													}
													if (_lastCatchableEvent === "blur"
															&& (angular
																	.isUndefined(closestTableParent) || closestTableParent
																	&& !closestTableParent
																			.hasClass("calendar-date"))
															&& angular
																	.isDefined(_getPicker())) {
														_getPicker().hide();
													}
												});
							}
	...
	angular
			.module("nDaterangepicker")
			.directive(
					"shorthandDateRangePicker",
					function($log) {
						$log
								.warn("shorthandDateRangePicker directive might not work as expected!");
						return {
							restrict : "E",
							scope : {
								identifier : "@",
